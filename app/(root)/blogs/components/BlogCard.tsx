...
const BlogCard = ({author, createdAt, title, description, id, blogBanner, authorImage, useAgent}: blogCardProps) => {
  return (
    <div className="w-full flex flex-col gap-2 cursor-pointer group">
      <div className="md:aspect-square xl:h-[15rem] lg:h-[14rem] sm:h-[13rem] h-[12.5rem] aspect-video flex items-center justify-center overflow-hidden rounded">
        <div className='w-full h-full relative bg-neutral-600'>
          ...
          {!useAgent && <div className="absolute bottom-0 left-0 w-full bg-black/30 p-4 text-white flex items-center justify-between">
            <div>
      ...
      <div className='lg:mt-3 mt-2 flex gap-2 flex-col'>
        ...
        {!useAgent && <p className='line-clamp-2 text-gray-400'>{description}</p>}
        <Link href={`/blogs/${id}`} className='flex items-center gap-2 font-semibold hover:text-yellow-400 text-sm mt-3'>
          CONTINUE READING
