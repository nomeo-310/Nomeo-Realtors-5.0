...
const TextArea = React.forwardRef<HTMLTextAreaElement, textareaProps>(({className, icon:Icon, ...props}, ref) => {
  return (
    <div className={cn('rounded bg-gray-200 text-neutral-600 w-full relative h-[14rem]', className)}>
      <textarea className={`cursor-pointer rounded bg-inherit w-full h-full md:p-4 p-3.5 focus:outline-none resize-none lg:text-lg ${Icon ? 'pl-10 md:pl-12 lg:pl-12' : ''}`} {...props} ref={ref}>
      </textarea>
