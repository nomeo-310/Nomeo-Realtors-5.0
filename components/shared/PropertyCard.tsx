...
type propertyCardProps = Omit<propertyProps, 'mainFees'| 'optionalFees' | 'intro' | 'description'| 'verifiedamenities' | 'propertyImageList' | 'nearbyPlaces' > & { hideTag: boolean, agentDisplay: boolean, hideAgentInCharge?: boolean}

const PropertyCard = ({hideAgentInCharge, agentDisplay, hideTag, id, propertTag, propertyImage, propertySize, propertyCost, baths, beds, title, toilets, agentInCharge, agentInChargeImage, liked, city, state, saved, monthlyRent}:propertyCardProps) => {
  ...
  return (
    <div className="w-full flex flex-col gap-2 cursor-pointer group" onClick={() => router.push(`/property/${id}`)}>
      ...
      <div>
        <p className='text-lg line-clamp-1 mb-1 font-semibold'>{title}</p>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <HiOutlineMapPin size={22}/>
            <p className='line-clamp-1'>{city}, {state}.</p>
          </div>
